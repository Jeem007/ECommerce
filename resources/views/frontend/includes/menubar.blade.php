				...
				<ul class="nav nav-pills">
					...
					@if(Auth::check())
						...
						<li class="nav-item dropdown nav-item-left-border d-none d-sm-block">
							<a class="nav-link" href="#" role="button" id="options" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
						@if(!is_null(Auth::user()->image))
						<img src="{{asset('images/user/' .Auth::user()->image)}}" class="rounded-circle" style=" width: 30px; height: 30px;">
						@else
						<img src ="{{asset('images/user/user.jpg')}}" class="rounded-circle" style=" width: 30px; height: 30px;">
						@endif
							{{ Auth::user()->name }}
						<i class="fas fa-angle-down"></i>
							</a>
							<div class="dropdown-menu" aria-labelledby="options">
								<a class="dropdown-item" href="{{route('Customer_Dashboard')}}"> View Profile</a>
		...
		<div class="header-nav-features">
			<div class="header-nav-feature header-nav-features-cart header-nav-features-cart-big d-inline-flex ml-2" data-sticky-header-style="{'minResolution': 991}" data-sticky-header-style-active="{'top': '78px'}" data-sticky-header-style-deactive="{'top': '0'}">
				...
				<div class="header-nav-features-dropdown" id="headerTopCartDropdown">
					...
					<li class="item">
							...
							<div class="product-details">
								...
								<form action = "{{route('cart.delete',$cart->id)}}" method = "POST">
									@csrf
								<button type="submit" title="Remove This Item" class="btn-remove"><i class="fas fa-times"></i>
								</button>
								</form>
					...
					<span class="label">Sorry! No Items Added in Cart</span>
					@else
